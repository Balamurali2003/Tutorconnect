...
import { fetchParentClasses } from '../../services/api';
import { Calendar, Clock, MapPin, User } from 'lucide-react';

export const ParentClassesPage: React.FC = () => {
  const [classes, setClasses] = useState<any[]>([]);

  useEffect(() => {
    fetchParentClasses().then(res => setClasses(res.classes || [])).catch(() => {});
  }, []);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Tuition Classes Schedule</h1>
        <p className="text-xs text-slate-500 mt-0.5">
          Weekly timetable of home tuition lessons for your child
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {classes.map((c) => (
            ...
            >
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">Tutor: {c.tutorName}</h3>
